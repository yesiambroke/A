...
interface LadderBuyModalProps {
  ...
  onRestore?: () => void;
  currentCoin: string;
  buildPumpBuyInstructions: (params: {
    mintAddress: string;
    buyAmount: string;
...
const LadderBuyModal: React.FC<LadderBuyModalProps> = ({
  ...
  // Modal state
  const [ladderBuyAmounts, setLadderBuyAmounts] = useState<Record<string, string>>({});
  const [ladderBuyAmount, setLadderBuyAmount] = useState('');
  const [ladderBuyDelay, setLadderBuyDelay] = useState('1');

  // Randomization state
  const [ladderBuyRandomPercentage, setLadderBuyRandomPercentage] = useState(60);
  const [ladderBuyMinPercentage, setLadderBuyMinPercentage] = useState(20);
  const [ladderBuyTargetAmount, setLadderBuyTargetAmount] = useState('');
  const [useTargetAmount, setUseTargetAmount] = useState(false);

  // Ladder buy execution state
  const [isLadderBuyRunning, setIsLadderBuyRunning] = useState(false);
  const [isLadderBuyCancelled, setIsLadderBuyCancelled] = useState(false);
  const [ladderBuyAbortController, setLadderBuyAbortController] = useState<AbortController | null>(null);
  const [ladderBuyQueue, setLadderBuyQueue] = useState<string[]>([]);
  const [processedLadderBuyWallets, setProcessedLadderBuyWallets] = useState<string[]>([]);
  ...
  React.useEffect(() => {
    if (isOpen) {
      ...
      setIsLadderBuyRunning(false);
      setIsLadderBuyCancelled(false);
  ...
  const calculateRandomizedAmount = (walletId: string) => {
    ...
    if (useTargetAmount && ladderBuyTargetAmount) {
      // Distribute target amount proportionally across wallets based on their SOL balance
      const targetAmount = parseFloat(ladderBuyTargetAmount);
      if (!isNaN(targetAmount) && targetAmount > 0) {
        ...
        if (totalBalance > 0) {
          ...
          const minPercent = ladderBuyMinPercentage / 100;
          const maxPercent = ladderBuyRandomPercentage / 100;
  ...
  const handleMainAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLadderBuyAmounts(newAmounts);
  };
  ...
  const handleIndividualAmountChange = (walletId: string, value: string) => {
    const newAmounts = { ...ladderBuyAmounts };
  ...
  const startLadderBuy = async (walletsToProcess?: string[]) => {
    const queueToUse = walletsToProcess || ladderBuyQueue;
    if (queueToUse.length === 0 || isLadderBuyRunning) return;

    setIsLadderBuyRunning(true);
    setIsLadderBuyCancelled(false);
    setLadderBuyQueue(queueToUse); // Ensure queue is set

    const abortController = new AbortController();
    setLadderBuyAbortController(abortController);

    onToast(`Starting ladder buy for ${queueToUse.length} remaining wallets...`);
    ...
    try {
      for (const walletId of queueToUse) {
        ...
        const amount = parseFloat(ladderBuyAmounts[walletId] || '0');
        if (amount <= 0) {
          setProcessedLadderBuyWallets(prev => [...prev, walletId]);
          continue;
        }
        ...
        try {
          // Execute actual buy trade via WSS
          onToast(`🪙 ${walletName}: Buying ${amount} SOL...`);
        ...
        const delayMs = (parseFloat(ladderBuyDelay) || 1) * 1000;
      ...
      setLadderBuyQueue(prev => prev.filter(id => !processedLadderBuyWallets.includes(id)));

      onToast('✅ Ladder buy completed successfully!');

      setIsLadderBuyRunning(false);
      setLadderBuyAbortController(null);
    } catch (error) {
      console.error('Ladder buy error:', error);
      onToast('❌ Ladder buy failed');
      setIsLadderBuyRunning(false);
      setLadderBuyAbortController(null);
    }
  };
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <div className="text-left">
              <div className="text-green-100 font-mono text-xs font-medium">Ladder Buy</div>
              <div className="text-green-500/70 font-mono text-[10px]">
                {isLadderBuyRunning ? `${processedLadderBuyWallets.length}/${selectedWallets.length}` : 'Minimized'}
              </div>
            </div>
    ...
    <div className="fixed inset-0 flex items-center justify-center z-[9999]" onClick={handleBackdropClick}>
      <div className="bg-black/90 border border-green-500/30 rounded-lg shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col"
        ...
        <div className="flex items-center justify-between p-3 border-b border-green-500/20">
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-baseline gap-2">
              <h2 className="text-green-300 font-mono font-semibold text-sm">Ladder Buy</h2>
              <span className="text-green-500/60 text-xs font-mono">• Sequential processing</span>
            </div>
          </div>
        ...
        <div className="flex-1 flex min-h-0">
          ...
          <div className="flex-1 p-6 border-r border-green-500/20 overflow-y-auto">
              ...
              <div className="max-h-[420px] overflow-y-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {selectedWallets.map((walletId, index) => {
                    const wallet = connectedWallets.find(w => w.id === walletId);
                    const amount = ladderBuyAmounts[walletId] || '';
                    const isProcessed = processedLadderBuyWallets.includes(walletId);
                    const isQueued = ladderBuyQueue.includes(walletId);
          ...
          <div className="w-80 p-6 overflow-y-auto">
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <h4 className="text-green-300 font-mono font-medium text-sm">Settings</h4>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-end gap-2">
                        <div className="flex-1">
                          ...
                          <input
                            type="number"
                            value={ladderBuyMinPercentage}
                            onChange={(e) => setLadderBuyMinPercentage(Math.max(1, Math.min(100, parseInt(e.target.value) || 20)))}
                        ...
                        <div className="flex-1">
                          ...
                          <input
                            type="number"
                            value={ladderBuyRandomPercentage}
                            onChange={(e) => setLadderBuyRandomPercentage(Math.max(1, Math.min(100, parseInt(e.target.value) || 60)))}
                            className="w-full rounded border border-green-500/30 bg-black/60 px-2 py-1.5 text-green-100 focus:outline-none focus:border-green-400 text-xs"
                            disabled={isLadderBuyRunning}
                            min="1"
                            max="100"
                        ...
                        <button
                          onClick={() => {
                            ...
                            setLadderBuyAmount(amountsArray.join(','));
                          }}
                    ...
                    {useTargetAmount && (
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          type="text"
                          value={ladderBuyTargetAmount}
                          onChange={(e) => setLadderBuyTargetAmount(e.target.value)}
                          placeholder="5.0"
                  ...
                  <div className="pt-2 border-t border-green-500/20">
                    <div className="flex items-center gap-3">
                      ...
                      <input
                        type="text"
                        value={ladderBuyDelay}
                        onChange={(e) => setLadderBuyDelay(e.target.value)}
                        placeholder="1"
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-green-100 font-mono">
                        {Object.values(ladderBuyAmounts).reduce((sum, amount) => sum + (parseFloat(amount) || 0), 0).toFixed(3)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-green-500/70 font-mono">Duration</span>
                      <span className="text-green-100 font-mono">
                        ~{Math.max(1, (selectedWallets.length - 1) * parseFloat(ladderBuyDelay || '1'))}s
                      </span>
                    </div>
                    <div className="flex justify-between">
        ...
        <div className="border-t border-green-500/20 p-4">
          <div className="flex gap-3">
            ...
            >
              {isLadderBuyRunning ? 'Running...' : 'Cancel'}
            </button>
            {isLadderBuyRunning ? (
              <button
                onClick={async () => {
                  // Pause - cancel current process but keep progress
                  setIsLadderBuyCancelled(true);
                  if (ladderBuyAbortController) {
                    ladderBuyAbortController.abort();
                  }
                  // Wait a bit for the abort to take effect
                  await new Promise(resolve => setTimeout(resolve, 100));
                  setIsLadderBuyRunning(false);
                  onToast('Ladder buy paused');
                }}
            ...
            ) : processedLadderBuyWallets.length > 0 ? (
              <button
                onClick={() => {
                  // Resume - start with remaining wallets, reset cancelled state
                  const remainingWallets = selectedWallets.filter(id => !processedLadderBuyWallets.includes(id));
                  setIsLadderBuyCancelled(false); // Reset cancelled state
                  setLadderBuyAbortController(null); // Reset abort controller
                  startLadderBuy(remainingWallets); // Pass wallets directly
                }}
              ...
              >
                ...
                Resume ({selectedWallets.filter(id => !processedLadderBuyWallets.includes(id)).length})
              </button>
            ) : (
              <button
                onClick={() => {
                  startLadderBuy(selectedWallets.slice());
                }}
                disabled={selectedWallets.length === 0 || !Object.values(ladderBuyAmounts).some(amount => amount && parseFloat(amount) > 0)}
